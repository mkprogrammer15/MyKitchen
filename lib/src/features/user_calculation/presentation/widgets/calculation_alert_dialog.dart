...
class _CalculationAlertDialogState extends State<CalculationAlertDialog> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FirebaseCounterBloc, FirebasecounterState>(builder: (context, state) {
      if (state is FirebasePartsCalculatedState) {
        return AlertDialog(
          title: Text(
            '${AppLocalization.of(context)!.getTranslatedValues('Die Küchenmontage beträgt ca.')}${state.kitchenPrice} €',
          ...
          actions: [
            TextButton(
                ...
                child: Text(AppLocalization.of(context)!.getTranslatedValues('Abbrechen'))),
            TextButton(
              onPressed: () {
                Navigator.pushNamed(context, 'client_form_screen');
              },
              child: Text(AppLocalization.of(context)!.getTranslatedValues('Zur Zusammenfassung')),
            )
          ],
        );
      } else {
